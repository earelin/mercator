...
- **Configuration:** environment-driven (DB connection, BOE rate limits, daily-schedule cron
  expression, cache location).
...
- **Backups:** nightly `pg_dump` to object storage; the BOE is always re-ingestable as a
  disaster fallback.
- **Network:** the server exposes only the **read-only** API; there is no ingest endpoint to
  isolate.
