...
#   1. Markdown formatting/style   -> markdownlint-cli2          (cmd: markdown)
#   2. Links (relative + anchors, and external URLs) -> lychee   (cmd: links)
...
# ./scripts/api-conformance.sh (schemathesis), kept out of this pipeline deliberately.
#
# Run everything:         ./scripts/ci.sh            (or: ./scripts/ci.sh all)
# Run one/some checks:    ./scripts/ci.sh markdown links openapi
# List the checks:        ./scripts/ci.sh --help
# Skip external links:    CHECK_EXTERNAL=0 ./scripts/ci.sh
...
usage() {
  cat <<EOF
Usage: ./scripts/ci.sh [check ...]

Runs the local CI pipeline. With no arguments (or "all"), runs every check.
Otherwise runs only the named checks, in the order given.
...
Examples:
  ...
  ./scripts/ci.sh markdown links        # run just those two
  CHECK_EXTERNAL=0 ./scripts/ci.sh links
EOF
}

# --- Collect Markdown files (working tree), excluding generated/vendor dirs ---
# Lazily populated (only the markdown/links/mermaid checks need it).
MD_FILES=()
MD_COLLECTED=0
collect_md() {
  [ "$MD_COLLECTED" -eq 1 ] && return 0
  MD_COLLECTED=1
...
check_markdown() {
  ...
  collect_md
  if [ "${#MD_FILES[@]}" -eq 0 ]; then ok "no Markdown files"; return; fi
...
# Resolve the requested checks (default: all).
if [ "$#" -eq 0 ] || [ "${1:-}" = "all" ]; then
  REQUESTED=("${CHECKS[@]}")
else
  REQUESTED=("$@")
  for name in "${REQUESTED[@]}"; do
    if [[ " ${CHECKS[*]} " != *" $name "* ]]; then
      err "unknown check: $name"; echo; usage >&2; exit 2
    fi
  done
fi
