...
- ADRs: [0002 — Prefer structured text/XML over PDF](../architecture/0002-txt-php-over-pdf-parsing.md)
...
- **Cache** the raw response to disk/object storage keyed by `borme_id`; subsequent runs
  read the cache instead of re-downloading.
- Apply **rate-limiting** (≤1–2 req/s for backfill), **exponential backoff** on 429/5xx, and
  a descriptive **User-Agent**.
- **Fallback order on failure:** `txt.php` (`url_html`, strip HTML chrome) → **PDF**
  (`url_pdf`, extract text). PDF remains the authentic last resort.
...
- [ ] HTTP fetcher with rate limiter, backoff, and User-Agent.
- [ ] XML document fetch + parse (`<metadatos>` + `<texto>` paragraphs), encoding-safe.
- [ ] Raw-response cache (disk) keyed by `borme_id`, read-through.
- [ ] Fetch-error recording integrated with `borme_log`.
- [ ] Fallback chain: `txt.php` HTML-strip, then PDF text extraction.
- [ ] Optional object-storage cache backend.
